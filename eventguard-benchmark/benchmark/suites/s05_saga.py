...
class SagaSuite(Suite):
    ...
    def execute(self) -> None:
        ...
        self.add("approval_pending", "REFUND(150>100) 挂起审批单 PENDING", approval is not None,
                 expected="PENDING REFUND", actual=str(approval.get("approvalId"))[:8] if approval else "none")

        # 人工审批通过 → saga 继续执行 NOTIFY_DELAY → COMPLETED
        if approval:
            r, _ = self.ctx.client.post(
                f"/approvals/{approval['approvalId']}/approve",
        ...
        self.wait_until(lambda: self._notified(oid_a, "NOTIFY_DELAY"), timeout=20, interval=0.5)
        ...
        self.add("notify_delay", "notification_log 含 NOTIFY_DELAY", self._notified(oid_a, "NOTIFY_DELAY"),
                 expected="≥1", actual=str(len(self.ctx.db.notification_log(oid_a))))
        ...
        self.wait_until(lambda: self._status(oid_b) == "PAID", timeout=20, interval=0.2)
        ...
        self.wait_until(lambda: self._notified(oid_b, "NOTIFY_DELAY"), timeout=25, interval=0.5)
        ...
        self.add("inventory_comp_notify", "notification_log 含 NOTIFY_DELAY", self._notified(oid_b, "NOTIFY_DELAY"),
                 expected="≥1", actual=str(len(self.ctx.db.notification_log(oid_b))))
        self.add("inventory_status_paid", "库存失败订单状态保持 PAID", self._status(oid_b) == "PAID",
                 expected="PAID", actual=self._status(oid_b) or "—")

        success = (self._has_comp_event(oid_a, "REFUND") and self._notified(oid_a, "NOTIFY_DELAY")
