...
幂等是另一条线：客户端生成的 `commandId` 作为主键落 `command_log`，与事件写入同事务，重复提交返回首次结果而不产生第二条事件。
...
| `command_log` 主键幂等 | 网络重试导致重复事件 |
| `idempotent_consumers` 去重 + 版本单调守卫 | at-least-once 重复投递、乱序导致状态倒退 |
| 读己写按版本轮询（50ms / 2s 超时） | 投影滞后时把过期数据当成功返回 |

超时后抛 `ProjectionLagException` 让前端提示「数据同步中」，宁可如实告知，也不返回旧数据假装成功。

### 5. 分层检测，规则与模型各就各位

业务规则（金额偏离 3σ / 重复支付 / 状态跳跃 / 高频下单 / 库存超卖）与流程规则（停滞 / 死循环）放在 Java 侧，因为要读事件库算用户历史基线；统计模型（IsolationForest 事件级、HMM 序列级）放 Python 侧，经 HTTP 反向调用规则接口后合并结论。告警经 Kafka 到 WebSocket 秒级推到看板，断线重连按告警 ID 去重补拉。
...
Kafka 消费失败退避重试 3 次后进死信队列而非丢弃或死循环；告警发布失败退避重试；Saga 编排器重启后从数据库的 PENDING 审批单重放在途补偿；Debezium 有健康检查；网关凭证缺失时优雅降级返回失败原因而非崩溃。混沌 profile 会定期随机杀容器，验证故障下能恢复。
...
Vue3 管理台
  ...
  │                     ┌─────────────┼─────────────┬──────────────┐
  │                     ▼             ▼             ▼              ▼
  │            OrderViewProjection SagaTrigger  Python 检测   失败 3 次 → DLT
  │            (去重表 + 版本守卫)  (补偿编排)  (规则+IF+HMM)
  │                     │                           │
  │  读查询 ◀──── order_view (读己写 50ms / 2s)      │ 告警
  │                                                 ▼
  └─ WebSocket ◀── AnomalyWebSocketHandler ◀── Kafka anomaly-alerts
                          │
                    RecentAlertsBuffer (环形 100) ──▶ GET /alerts/recent
```
...
- **指标**：后端 `/actuator/prometheus` 暴露 `eventguard.*`（命令延迟与吞吐、Saga、告警、支付回调、限流拒绝、投影计数），AI 服务暴露 `eventguard_ai_*`（检测吞吐与延迟、NL 查询）。Prometheus 抓取两端，Grafana（`:3001`）预置看板，Alertmanager 覆盖服务不可用与 5xx 错误率。
- **日志**：Loki + promtail 采集各容器日志，Grafana 已配好数据源。
- **数据**：`scripts/backup-db.sh`（pg_dump custom 格式，保留 14 天）、`scripts/retain-events.sh`（归档 90 天前事件，默认 dry-run）。
...
- 先榨干规则本身的空间：金额偏离阈值从 3σ 下调并按品类分桶，当前漏检样本大多卡在 2.5σ 附近。
...
- 异常告警的完整历史检索接口，替代当前最近 100 条的环形缓冲。
- WebSocket 心跳与半开连接剔除，多实例广播经 Redis Pub/Sub。
- 鉴权补齐刷新令牌与主动吊销，权限变更即时生效而非重新登录。
- 限流从固定窗口升级为滑动窗口，消除窗口边界的两倍突发。
- 中文查询扩展到更多意图与多轮追问，并对参数抽取做澄清反问。

## 验证

```bash
mvn -q test                                    # 后端 164 项（4 项需 Docker 跑 Testcontainers）
cd eventguard-ai && pytest -q                  # AI 服务 125 项
...
- 完整设计文档：[docs/架构设计/eventguard-design.md](docs/架构设计/eventguard-design.md)
- 架构拓扑图：[docs/架构设计/architecture.svg](docs/架构设计/architecture.svg)
- 架构评审记录：[docs/架构设计/architecture-review-2026-08.md](docs/架构设计/architecture-review-2026-08.md)
- 本地开发启动顺序：[docs/使用指南/local-development.md](docs/使用指南/local-development.md)
- 逐场景走查脚本：[docs/使用指南/demo-script.md](docs/使用指南/demo-script.md)
