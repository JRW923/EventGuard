...
ai_health() {
  local code
  code=$(curl -fsS -o /dev/null -w "%{http_code}" -H "X-API-Key: ${API_KEY}" "http://localhost:${AI_PORT}/health" 2>/dev/null || echo "000")
  [[ "$code" = "200" ]]
}
...
# 命令端可写：POST /orders 应返回 200（不依赖 Kafka，直接写 PG 事件库）
command_write_ok() {
  local code
...
# 规则引擎兜底可用：POST /anomaly/rules/evaluate 由命令端本地处理，不依赖 AI 网络
rule_engine_ok() {
  local code
  code=$(curl -fsS -o /dev/null -w "%{http_code}" -X POST "http://localhost:${SERVER_PORT}/anomaly/rules/evaluate" \
    -H "Content-Type: application/json" -H "X-API-Key: ${API_KEY}" \
    -d '{"aggregateId":"chaos-probe","eventType":"OrderCreatedEvent"}' 2>/dev/null || echo "000")
  [[ "$code" = "200" ]]
}
