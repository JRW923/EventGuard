...
- **Kafka 消费组（不止投影一个）**：Kafka 的"消费组"是带独立 offset 的订阅者，同一主题可被多个组各自完整消费一遍、互不影响。本项目常驻**四个**组：`order-view-projection`（`OrderViewProjection.java:48`，消费 `domain-events` 建读模型）、`saga-trigger`（`SagaTrigger.java:59`，同样消费 `domain-events` 触发补偿 Saga）、`ai-event-detector`（`eventguard-ai/app/kafka_consumer.py:46`，Python 侧手动 `KafkaConsumer`，同样消费 `domain-events` 跑异常检测）、`anomaly-ws`（`AnomalyAlertConsumer.java:39`，消费 `anomaly-alerts` 推 WebSocket）。其中 `domain-events` 被 **三个组同时独立消费**是有意为之——投影卡住不会拖慢 Saga、Saga 重放也不影响投影、AI 检测重启也不回退业务位点，正是事件流支撑"一个写、多个读"的物理基础。
  - **别漏掉 Python 侧的 `ai-event-detector`**：判断是不是消费组只看"有没有自己的 `group.id`、独立 offset、参与 rebalance"，与用什么语言、是否声明式无关。它用 kafka-python 手动 `KafkaConsumer(group_id=...)`（`config.py:10` 默认 `ai-event-detector`），有自己的位点提交与 `eventguard_ai_consumer_lag` 积压指标（`kafka_consumer.py:110-122`），`kafka-consumer-groups.sh --list` 照样列得出来；Java 代码里 grep `@KafkaListener` 搜不到它，那是视角造成的遗漏，不是技术上不算。它另有两处和其他组不同：① **不写 `idempotent_consumers` 表**，去重靠 `anomaly_id` 主键与 `alert_dedup.py`，所以从 DB 去重视角也容易漏；② **只有死信发布成功后才提交原 offset**（`kafka_consumer.py:149`），防止毒消息被跳过丢失。它是旁路只读消费者——不改订单状态、不参与读己写链路，只消费事件跑检测再往 `anomaly-alerts` 发告警。
  - **三组名字别混**：`OrderViewProjection` 里的常量 `CONSUMER_GROUP = "order-view"`（`OrderViewProjection.java:28`）**不是 Kafka 组**，而是写进 `idempotent_consumers` 表的 DB 级去重键（`tryMarkProcessed` 用它判定"这条事件我投影过没有"），与 Kafka `groupId` 作用域不同，别混；该表当前只有 `order-view`、`saga-trigger` 两个写入方。此外 `application.yml` 的 `spring.kafka.consumer.group-id=debug-consumer` 只是兜底默认值，业务监听器已全部显式覆盖、实际不生效；DLT 重放时会临时起一个 `dlt-replay` 组（`DltReplayController.java:75`），非常驻。
- **Kafka 分区与分配策略（两件事别混）**：
