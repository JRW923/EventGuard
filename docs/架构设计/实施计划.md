...
本计划基于 `系统设计.md` 第 8 章 MVP 路线图展开，仅覆盖 MVP 必做项（M1-M5）。V2 能力（HMM、Text-to-SQL、ReAct Agent、Saga 自动执行、Jepsen）不在本计划内，仅在文末"V2 待办"列出。
...
> V2 主线（端点鉴权 V2.1–V2.5 + AI 异步化 V2.7 + 补偿 Bean 化 V2.6 + 导入清理 V2.8 + Git LFS V2.9 + WS 鉴权整合修复 V2.10）已于 2026-07-25 合并 main，详见 `docs/superpowers/plans/2026-07-24-第二版-已知上限.md`。
