...
> 本文是风险审查和升级路线，不是当前实现清单。当前实现请看[当前架构基线](当前架构基线.md)；历史计划请看[实施计划](实施计划【归档】.md)。审查中标记为“建议/缺口”的内容，只有在实施记录明确标记落地后才视为现状。
...
- **本地调试**：新增 `eventguard-server/src/main/resources/application-local.yml`，IDEA 使用 `local` profile 时将 Kafka、PostgreSQL 指向 `localhost`；新增 `docs/使用指南/本地运行.md` 和 `eventguard-ai/.env.local.example`，明确 IDEA、FastAPI、Vite 的启动顺序。未改动生产 Docker 配置。
