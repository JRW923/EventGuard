# EventGuard 演示：Cloudflare Tunnel HTTPS

## 背景与约束

- 域名 `jrwdev.site` / `www.jrwdev.site` 在腾讯云购买，但**未做 ICP 备案**。
- 源站服务器在腾讯云广州（公网 IP `111.231.101.27`），**不想迁移到境外**。
- 个人静态站由 Cloudflare Pages 承载：`jrwdev.site` 为主域，`www.jrwdev.site` 跳转到主域。
- 目标：用 `eventguard.jrwdev.site` + HTTPS 访问 EventGuard，且不触发备案拦截。
...
即使开 Cloudflare 橙色云，Cloudflare 回源到腾讯 IP 时 `Host` 仍是 `eventguard.jrwdev.site`（未备案）→ 同样被 302 拦截。
**橙色云代理本身绕不开备案。**
...
### 3. 绑定 EventGuard 子域名 DNS（必须删掉旧 A 记录）

Cloudflare 不允许同一主机名同时有 A 和 CNAME。若之前为 `eventguard.jrwdev.site` 建过指向腾讯 IP 的 A 记录，
先在控制台**删除**该 A 记录（仅关橙色云不够，仍会回源到 IP 被拦），再：

```bash
cloudflared tunnel route dns eventguard eventguard.jrwdev.site
```
...
`docker-compose.yml` 已含 `cloudflared` 服务（`restart: unless-stopped`），按 [config.yml](../../cloudflared/config.yml) 的 hostname 路由：
...
`config.yml` 把 `eventguard.jrwdev.site` 转发到 docker 内部 `eventguard-ui:80`（UI 的 nginx）。根域和 `www` 由 Cloudflare Pages 处理，不得再配置为 Tunnel Public Hostname。改完配置：
...
```bash
# EventGuard 项目页应为 200（Cloudflare 证书）
curl -I https://eventguard.jrwdev.site/

# WebSocket 升级（真实端点 /ws/anomalies，需登录 JWT 经 ?token= 校验）
# 先登录拿 token，再连 WS
TOKEN=$(curl -s -X POST https://eventguard.jrwdev.site/auth/login -H "Content-Type: application/json" \
  -d '{"username":"operator","password":"operator123456"}' \
  | python3 -c "import sys,json;print(json.load(sys.stdin)['token'])")
curl -i -N "https://eventguard.jrwdev.site/ws/anomalies?token=$TOKEN"
...
```bash
dig @1.1.1.1 eventguard.jrwdev.site +short   # → CNAME 到隧道；A 为 Cloudflare 边缘 IP（如 104.21.x / 172.67.x）
```
