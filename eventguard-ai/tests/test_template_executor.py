...
class TestTemplateExecutor:
    ...
    @pytest.mark.asyncio
    async def test_event_lookup_extracts_order_id_and_calls_get_order(self):
        ...
        data = await executor.execute_event_lookup("订单 11111111-1111-1111-1111-111111111111 当前状态是什么？")
    ...
    @pytest.mark.asyncio
    async def test_stats_aggregation_extracts_status_and_time_window(self):
        ...
        data = await executor.execute_stats_aggregation("昨天有多少 PAID 订单？")
    ...
    @pytest.mark.asyncio
    async def test_stats_aggregation_without_status_passes_none(self):
        """stats_aggregation 未匹配状态时 status=None（全状态聚合）。"""
        mock_backend = AsyncMock()
        mock_backend.get_stats.return_value = []
        executor = TemplateExecutor(backend_client=mock_backend)

        await executor.execute_stats_aggregation("昨天有多少订单？")
    ...
    async def test_trace_replay_extracts_order_id_and_calls_get_events(self):
        ...
        data = await executor.execute_trace_replay("订单 22222222-2222-2222-2222-222222222222 经历了哪些状态变更？")
